...
public class BattleField extends Application {
	...
	/*
	 * ////////////////////////////////////////////////////// START Start, not
	 * main, is called when the program starts up
	 * //////////////////////////////////////////////////////
	 */
	...
	public void SetStage() {
		...
		// Initialize background, valid position/target icons, & attack button
		// images
		...
		Field.addAll(Background, ValidAttack, InvalidAttack, MessageBoard,
	...
	public void CreateHeroes() {
		...
		Field.addAll(Warrior.Blood, Warrior.Poison, Warrior.Stun, Ranger.Blood,
				Ranger.Poison, Ranger.Stun, Mage.Blood, Mage.Poison, Mage.Stun,
				Priest.Blood, Priest.Poison, Priest.Stun);

		// Add their attacks and corresponding radio buttons to the Pane
		Field.addAll(Warrior.Attacks, Ranger.Attacks, Mage.Attacks,
				Priest.Attacks);
		Field.addAll(Warrior.getAttack(1), Warrior.getAttack(2),
				Warrior.getAttack(3), Warrior.getAttack(4),
				Ranger.getAttack(1), Ranger.getAttack(2), Ranger.getAttack(3),
				Ranger.getAttack(4), Mage.getAttack(1), Mage.getAttack(2),
				Mage.getAttack(3), Mage.getAttack(4), Priest.getAttack(1),
				Priest.getAttack(2), Priest.getAttack(3), Priest.getAttack(4));

		// Add the position changing radio buttons to the Pane
		Field.addAll(Warrior.MoveBack, Warrior.MoveForward, Ranger.MoveBack,
				Ranger.MoveForward, Mage.MoveBack, Mage.MoveForward,
	...
	public void CreateEnemies() {
		...
		// Add their status symbols to the Pane
		Field.addAll(EnemyWarrior.Blood, EnemyWarrior.Poison,
				EnemyWarrior.Stun, EnemyRanger.Blood, EnemyRanger.Poison,
				EnemyRanger.Stun, EnemyMage.Blood, EnemyMage.Poison,
				EnemyMage.Stun, EnemyPriest.Blood, EnemyPriest.Poison,
				EnemyPriest.Stun);

		// Add their attacks and the corresponding radiobuttons to the Pane
		Field.addAll(EnemyWarrior.Attacks, EnemyRanger.Attacks,
				EnemyMage.Attacks, EnemyPriest.Attacks);
		Field.addAll(EnemyWarrior.getAttack(1), EnemyWarrior.getAttack(2),
				EnemyWarrior.getAttack(3), EnemyWarrior.getAttack(4),
				EnemyRanger.getAttack(1), EnemyRanger.getAttack(2),
				EnemyRanger.getAttack(3), EnemyRanger.getAttack(4),
				EnemyMage.getAttack(1), EnemyMage.getAttack(2),
				EnemyMage.getAttack(3), EnemyMage.getAttack(4),
				EnemyPriest.getAttack(1), EnemyPriest.getAttack(2),
				EnemyPriest.getAttack(3), EnemyPriest.getAttack(4));
		...
		// Add their nameplates to the Pane
		Field.addAll(EnemyWarrior.Stats, EnemyRanger.Stats, EnemyMage.Stats,
				EnemyPriest.Stats);

		// Bind collective HP to the sum of all constituents
		EnemyHP = new SimpleIntegerProperty();
		EnemyHP.bind(EnemyWarrior.HP.add(EnemyRanger.HP).add(EnemyMage.HP)
				.add(EnemyPriest.HP));
	...
	public void FillHelperLists() {
		...
		// The X-Positions of all Dudes are stored here for reference when
		// changing position
		XPositions = new double[] { Hero4X, Hero3X, Hero2X, Hero1X, Enemy1X,
				Enemy2X, Enemy3X, Enemy4X };
	...
	public void AddListeners() {
		...
		for (Dude guy : Characters) {
			guy.MoveSet.selectedToggleProperty().addListener(
	...
	public void PickReadiestDude() {
		...
		for (Dude guy : ReadyDudes) {
			...
			if ((CurrentAttacker == null)
					&& (guy.getReadiness() >= readinessThreshold)) {
		...
		if (CurrentAttacker == null)
			...
			/*
			 * Once ready guy is picked, inflict bleeding, poison, & stun
			 * effects
			 */
			CheckHisStatus();
	...
	public void PrepareDefender() {
		...
		if (CurrentDefender == null)
			CurrentDefender = (CurrentAttacker.isEnemy()) ? (DudesInOrder
					.get(3)) : (DudesInOrder.get(4));
	...
	public void TurnAttackButtonOnOff() {
		...
		// target is alive
		else if (selectedAttack.isValidAttackPosition(CurrentAttacker
				.getPosition())
	...
	public void ProcessAttack() {
		...
		// Process normal damage calculations
			...
			int minAttackDamage = (int) Math.ceil(CurrentAttacker
					.getMinDamage() * (selectedAttack.getDamage() / 100.0));
			int maxAttackDamage = (int) Math.ceil(CurrentAttacker
					.getMaxDamage() * (selectedAttack.getDamage() / 100.0));
			...
			if (selectedAttack.Heals()) {
				...
				if (CurrentDefender.getHP() < 0) {
					CurrentDefender.setHP(0);
				}
	...
	public void PrepareLog() {
		...
		if (CurrentAttacker.isEnemy()) {
			...
			thisTurn.setValue(2,
					Boolean.toString(CurrentAttacker.canHit(maxPosition)));
			//Entry.append(CurrentAttacker.canHit(minPosition)).append(", ");
			thisTurn.setValue(3,
					Boolean.toString(CurrentAttacker.canHit(minPosition)));
			...
			thisTurn.setValue(
					5,
					Boolean.toString(CurrentAttacker.canHit(Ranger
							.getPosition()) && Ranger.isAlive()));
			//Entry.append(CurrentAttacker.canHit(Mage.getPosition())&& Mage.isAlive()).append(", ");
			thisTurn.setValue(
					6,
					Boolean.toString(CurrentAttacker.canHit(Mage.getPosition())
							&& Mage.isAlive()));
			//Entry.append(CurrentAttacker.canHit(Priest.getPosition())&& Priest.isAlive()).append(", ");
			thisTurn.setValue(
					7,
					Boolean.toString(CurrentAttacker.canHit(Priest
							.getPosition()) && Priest.isAlive()));

			// Do foes have status conditions?
			//Entry.append(
					//Warrior.isBleeding() || Warrior.isPoisoned()
							//|| Warrior.isStunned()).append(", ");
			...
			thisTurn.setValue(9, Boolean.toString(Ranger.isBleeding() || Ranger.isPoisoned()
							|| Ranger.isStunned()));
			//Entry.append(Mage.isBleeding() || Mage.isPoisoned() || Mage.isStunned()).append(", ");
			thisTurn.setValue(
					10,
					Boolean.toString(Mage.isBleeding() || Mage.isPoisoned()
							|| Mage.isStunned()));
			//Entry.append(Priest.isBleeding() || Priest.isPoisoned()|| Priest.isStunned()).append(", ");
			thisTurn.setValue(
					11,
					Boolean.toString(Priest.isBleeding() || Priest.isPoisoned()
							|| Priest.isStunned()));
			...
			// Is current attacker in a valid attack position
			Entry.append(
					CurrentAttacker.canAttack(CurrentAttacker.getPosition()))
					.append(", ");
			...
			// Are foes alive and in range of an attack?
			Entry.append(
					CurrentAttacker.canHit(EnemyWarrior.getPosition())
							&& EnemyWarrior.isAlive()).append(", ");
			Entry.append(
					CurrentAttacker.canHit(EnemyRanger.getPosition())
							&& EnemyRanger.isAlive()).append(", ");
			Entry.append(
					CurrentAttacker.canHit(EnemyMage.getPosition())
							&& EnemyMage.isAlive()).append(", ");
			Entry.append(
					CurrentAttacker.canHit(EnemyPriest.getPosition())
							&& EnemyPriest.isAlive()).append(", ");

			// Do foes have status conditions?
			Entry.append(
					EnemyWarrior.isBleeding() || EnemyWarrior.isPoisoned()
							|| EnemyWarrior.isStunned()).append(", ");
			Entry.append(
					EnemyRanger.isBleeding() || EnemyRanger.isPoisoned()
							|| EnemyRanger.isStunned()).append(", ");
			Entry.append(
					EnemyMage.isBleeding() || EnemyMage.isPoisoned()
							|| EnemyMage.isStunned()).append(", ");
			Entry.append(
					EnemyPriest.isBleeding() || EnemyPriest.isPoisoned()
							|| EnemyPriest.isStunned()).append(", ");
		}
	...
	public void UpdateLog() {
		...
		default:
			System.out
					.println("Error recording log. Current attacker doesn't know his name.");
		}
	...
	public void SaveWarriorLog() {
		...
		if (logFile.exists() && !logFile.isDirectory()) {
			try {
				...
				System.out
						.println("Error recording the warrior log. Here are the results:\n");
	...
	public void SaveRangerLog() {
		...
		if (logFile.exists() && !logFile.isDirectory()) {
			try {
				...
				// Inform user of error
				System.out
						.println("Error recording the ranger log. Here are the results:\n");
	...
	public void SaveMageLog() {
		...
		if (logFile.exists() && !logFile.isDirectory()) {
			try {
				...
				// Inform user of error
				System.out
						.println("Error recording the mage log. Here are the results:\n");
	...
	public void SavePriestLog() {
		...
		if (logFile.exists() && !logFile.isDirectory()) {
			try {
				...
				// Inform user of error
				System.out
						.println("Error recording the Priest log. Here are the results:\n");
	...
	public boolean NobodyHasWonYet() {
		...
		if (!Warrior.isAlive() && !Ranger.isAlive() && !Mage.isAlive()
				&& !Priest.isAlive()) {
			// Display Result
			Display("\n\nENEMIES WIN\n\n");
		...
		else if (!EnemyWarrior.isAlive() && !EnemyRanger.isAlive()
	...
	public int RandomInRange(int min, int max) {
		HighQualityRandom random = new HighQualityRandom(
				System.currentTimeMillis() * (max + 5));
